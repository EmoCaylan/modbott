...
if(kullanici.voice.channel) kullanici.voice.setMute(true).catch(err => console.error("Kullanıcı ses kanalında değil!"));
...
data.set(`cezainfo.${cezaID}.${message.guild.id}`, { mod: message.author.id, sebep: sebep, kisi: kullanici.id, id: cezaID, zaman: voiceatılma, komut: "Voice Mute Atıldı" });
...
client.channels.cache.get(VMuteLog).send(
    new MessageEmbed()
.setTitle(`Bir Üye Sesli Kanallarından Engellendi ! Ceza Numarası : \`${cezaID}\``)
