...
data.set(`cezainfo.${cezaID}.${message.guild.id}`, { mod: message.author.id, sebep: sebep, kisi: kullanici.id, id: cezaID, zaman: jailatılma, komut: "Jaile Atıldı" });
...
message.react(doru)
kullanici.roles.cache.has(inventory.Penalties.booster) ? kullanici.roles.set([inventory.Penalties.booster, server.Penalties.JailRole]) : kullanici.roles.set([inventory.Penalties.JailRole])
kullanici.roles.cache.forEach(r => {
data.set(`${message.guild.id}.jail.${kullanici.id}.roles.${r.id}`, r.id )})
moment.locale("tr");
client.channels.cache.get(JailLog).send(new MessageEmbed().setTitle(`Bir Üye Cezalandırıldı ! Ceza Numarası : \`${cezaID}\``).setColor(inventory.Colors.Special).setFooter(message.author.tag, message.author.avatarURL({dynamic:true})).setTimestamp().setDescription(`Bir üye cezalandırıldı, **${kullanici.user.tag}** \`${sebep}\` sebebiyle \`${time}\` boyunca metin kanalları ve sesli kanallardan yasaklandı. ${inventory.Emoji.True}\n\nYetkili: ${message.author} (\`${message.author.id}\`)\nKullanıcı: ${kullanici.user} (\`${kullanici.user.id}\`)\nSüre: \`${time}\` \nSebep: \`${sebep}\` \n Jail Atılma Saati: \`${jailatılma}\` \n Jail Bitiş Saati: \`${jailbitiş}\` `));
setTimeout(async () =>{
...
if(roller){kullanici.roles.add(roller).catch(err => console.error("Kişiye gerekli rolleri veremedim veya üzerinde herhangi bir rol bulunmuyordu!"))};
