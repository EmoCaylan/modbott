...
if (!member) return message.channel.send(new MessageEmbed().setColor(inventory.Colors.NoEmbed).setDescription(`${message.author}, lütfen bir kullanıcı etiketle !`).setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))).then(x => x.delete({timeout: 6500}));
          
let mute = message.mentions.members.first() || message.guild.members.cache.find(r => r.id === args[0]);
if (!mute) { new MessageEmbed().setColor(inventory.Colors.NoEmbed).setDescription(`${message.author}, lütfen mute atmam gereken kullanıcı belirt !`).setAuthor(message.author.name, message.author.avatarURL({dynamic: true})).then(x => x.delete({timeout: 6500}));;
} else {
if (mute.roles.highest.position >= message.member.roles.highest.position) 
              {
return message.channel.send(new MessageEmbed().setColor(inventory.Colors.NoEmbed).setDescription(`Bu Kullanıcı Senden Üst/Aynı Pozisyonda.`).setAuthor(message.author.name, message.author.avatarURL({dynamic: true}))).then(x => x.delete({timeout: 6500}));;
} else {
    let zaman = args[1]   
    .replace("sn", "s")
    .replace("dk", "m")
...
data.set(`cezainfo.${cezaID}.${message.guild.id}`, { mod: message.author.id, sebep: sebep, kisi: member.id, id: cezaID, zaman: muteatılma, komut: "Mute Atıldı" });
...
new MessageEmbed()
.setTitle(`Bir Üye Metin Kanallarından Engellendi ! Ceza Numarası : \`${cezaID}\``)
